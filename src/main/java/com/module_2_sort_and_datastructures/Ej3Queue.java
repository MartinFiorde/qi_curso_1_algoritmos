...
class QueueTeacherExample<T> implements QueueCourseImplementation<T> {
    ...
    public void enqueue(T item) {
        ...
        this.tail.next = newTail; // NOTA falla menor en el ejemplo del curso. cuando se agrega el primer elemento, esta operación no debería ejecutarse porque genera un next autoreferencial. al agregar un segundo elemento se soluciona esta inconsistencia
        this.tail = newTail;
    }
    ...
    public T dequeue() {
        ...
        if (length == 0) tail = null; // opcional bookkeeping
